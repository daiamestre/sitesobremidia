-- ============================================================================
-- MIGRATION: 20261006_fix_playlist_rpc_media_table.sql
-- BUG FIX: get_player_playlist_for_screen usava public.midias (coluna 'nome')
-- em vez de public.media (coluna 'name'). Erro: "column ra.name does not exist"
-- ============================================================================
...
BEGIN
    ...
    -- CORREÇÃO: usa public.media (tem coluna 'name', 'file_url', 'file_type')
    -- NÃO public.midias (que tem 'nome', 'object_key') — era o bug do erro
    -- "column ra.name does not exist (perhaps you meant ra.nome)"
    SELECT jsonb_agg(
        jsonb_build_object(
            'id', pi.id,
