-- Migration: 20261013_reconcile_audio_policy_playlist
-- Description: Reconciliação atômica e idempotente da política de áudio para public.playlists e RPC get_player_playlist_for_screen

BEGIN;

-- 1. Adicionar coluna audio_enabled em playlists com default false
ALTER TABLE public.playlists 
ADD COLUMN IF NOT EXISTS audio_enabled BOOLEAN DEFAULT false;

-- 2. Migração segura dos dados existentes a partir de screens (priorizando created_at ASC)
UPDATE public.playlists p
SET audio_enabled = COALESCE(
    (
        SELECT s.audio_enabled
...
-- 3. Garantir NOT NULL e DEFAULT false
ALTER TABLE public.playlists 
ALTER COLUMN audio_enabled SET DEFAULT false;

ALTER TABLE public.playlists 
ALTER COLUMN audio_enabled SET NOT NULL;

-- 4. Atualizar RPC get_player_playlist_for_screen para incluir playlists.audio_enabled no payload
...
-- 5. Registrar migration na tabela schema_migrations
INSERT INTO supabase_migrations.schema_migrations (version, name)
VALUES ('20261013', 'reconcile_audio_policy_playlist')
