-- Player RPC: entrega `days_of_week` como TEXTO ("0,1,2"), nao como array JSON.
-- Auditoria 2026-09-25: a coluna playlist_items.days e integer[] e o RPC devolvia um array JSON; o DTO do Player
-- (String) falhava com "Expected beginning of the string, but got [" e a sincronizacao INTEIRA caia no cache offline
-- assim que QUALQUER item tinha dias definidos (reproduzido no emulador). Texto e compativel com todos os Players ja
-- instalados (DTO String) e com o novo (aceita texto e array). NULL/'' = sem restricao de dias.
-- Unica mudanca em relacao a definicao em producao: a linha 'days_of_week'.
