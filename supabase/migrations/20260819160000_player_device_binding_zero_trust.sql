-- ============================================================================
-- MIGRATION: 20260819160000_player_device_binding_zero_trust.sql
-- DESCRIPTION: Implements ZERO TRUST Device Binding and secure playlist syncing
-- ============================================================================

-- 1. ADD DEVICE BINDING COLUMN TO SCREENS
ALTER TABLE public.screens ADD COLUMN IF NOT EXISTS bound_device_id TEXT;

-- 2. CREATE SECURE RPC FOR PLAYER SYNC
...
-- 3. FUNCTION FOR ADMINISTRATIVE RELEASE
...
BEGIN
    ...
    IF v_auth_uid IS NOT NULL THEN
        SELECT * INTO v_user_ctx FROM public.fn_get_user_security_context(v_auth_uid);
        IF v_user_ctx.status_ciclo_vida NOT IN ('ACTIVE', 'APPROVED') THEN RETURN FALSE; END IF;
    END IF;
    ...
    UPDATE public.screens SET bound_device_id = NULL WHERE id = p_screen_id;
    RETURN TRUE;
END;
$$;

-- 4. APPLY PERMISSIONS
GRANT EXECUTE ON FUNCTION public.get_player_playlist_for_screen TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.release_screen_device TO anon, authenticated, service_role;
