-- Widget Engine — Micro-gate W1 (correções de produção), 2026-09-25.
--
-- 1) fn_server_now(): hora do servidor para o relógio web se corrigir (o navegador não expõe o cabeçalho Date).
-- 2) external_links.embed_code: a tela Links Externos grava esta coluna, que NÃO existia (erro 42703 -> nenhum link
--    salvava; tabela com 0 linhas). Aditiva e nula.
-- 3) get_player_playlist_for_screen: itens de widget DESATIVADO deixam de ser enviados ao Player. Única mudança na
--    função (contrato do payload inalterado: o item simplesmente não vem, como se tivesse sido removido).
-- Reversível: DROP FUNCTION public.fn_server_now(); ALTER TABLE public.external_links DROP COLUMN embed_code;
--             e recriar a RPC sem o NOT EXISTS (definição anterior no histórico deste arquivo/ledger W1).

CREATE OR REPLACE FUNCTION public.fn_server_now()
RETURNS timestamptz
LANGUAGE sql
STABLE
SECURITY INVOKER
...
AS $fn$ SELECT now() $fn$;
GRANT EXECUTE ON FUNCTION public.fn_server_now() TO anon, authenticated;

ALTER TABLE public.external_links ADD COLUMN IF NOT EXISTS embed_code text;
