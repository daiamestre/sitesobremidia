 ...
 * Fonte única autoritativa para validar e declarar o status terminal de conclusão.
 */
...
export class TaskOrchestrator {
  ...
  /**
   * Executa uma tarefa completa de ponta a ponta com governança determinística.
   */
  async orchestrateTask(rawTask, actionHandlers = {}, options = {}) {
    ...
    // 6. Completion Authority Validation
    const completionValidation = CompletionAuthority.validateCompletion({
      task: canonicalTask,
      plan,
      executionResults,
      handoffs
    ...
    if (!completionValidation.completed) {
      return {
        task_id: canonicalTask.task_id,
        plan_id: plan.plan_id,
        status: completionValidation.status,
        error: `Autoridade de conclusão rejeitou fechamento da tarefa: ${completionValidation.errors.join('; ')}`,
        execution_results: executionResults,
        handoffs,
        started_at: startedAt,
        finished_at: new Date().toISOString()
    ...
    this.completedTasks.set(canonicalTask.task_id, finalRecord);

    return deepFreeze(finalRecord);
  }
}
