...
describe('Widget Engine W2 — fundação', () => {
  it('Galeria de Widgets: só modelos que o Player desenha podem ser criados; os demais ficam "Em breve"', () => {
    ...
    expect(prontos).toEqual(['clock-classic', 'clock-futurista', 'institutional-aviso', 'offer-destaque', 'rss-classic', 'weather-classic', 'weather-futurista']);
  });
  ...
  it('Galeria de Widgets: "Usar este modelo" só nos disponíveis', () => {
    ...
    expect(within(screen.getByTestId('template-advertising-campanha')).getByRole('button')).toBeDisabled();
  });
});
