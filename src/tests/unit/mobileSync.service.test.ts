...
describe('MobileSyncService', () => {
  ...
  it('syncOfflineData com fila vazia deve retornar success: true e syncedCount: 0', async () => {
    const result = await syncService.syncOfflineData('empresa-01', 'tecnico-01');
    expect(result.success).toBe(true);
    expect(result.syncedCount).toBe(0);
  });

  it('syncOfflineData deve sincronizar itens do tipo CHECKIN', async () => {
    storageService.enqueue('CHECKIN', { screenId: 's-01', latitude: -23.5505, longitude: -46.6333 });
    const result = await syncService.syncOfflineData('empresa-01', 'tecnico-01');
    expect(result.success).toBe(true);
  ...
  it('syncOfflineData deve sincronizar itens do tipo VISITA', async () => {
    storageService.enqueue('VISITA', { screenId: 's-02', dataAgendada: '2026-08-07' });
    const result = await syncService.syncOfflineData('empresa-01', 'tecnico-01');
    expect(result.success).toBe(true);
    expect(result.syncedCount).toBeGreaterThanOrEqual(1);
  });
  ...
  it('syncOfflineData deve limpar a fila após sincronização bem-sucedida', async () => {
    storageService.enqueue('CHECKIN', { screenId: 's-01', latitude: -23.5, longitude: -46.6 });
    await syncService.syncOfflineData('empresa-01', 'tecnico-01');
  ...
  it('syncOfflineData com múltiplos CHECKINs deve contabilizar todos', async () => {
    storageService.enqueue('CHECKIN', { screenId: 's-01', latitude: -23.5, longitude: -46.6 });
    storageService.enqueue('CHECKIN', { screenId: 's-02', latitude: -23.6, longitude: -46.7 });
    const result = await syncService.syncOfflineData('empresa-01', 'tecnico-01');
    expect(result.syncedCount).toBe(2);
  });

  it('syncOfflineData deve funcionar com múltiplos dispositivos (isolamento)', async () => {
    const [r1, r2] = await Promise.all([
      syncService.syncOfflineData('empresa-01', 'tecnico-A'),
      syncService.syncOfflineData('empresa-01', 'tecnico-B'),
    ]);
